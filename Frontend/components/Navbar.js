...
const MobileNestedDropdown = ({ title, categories, onLinkClick }) => {
  const [isOpen, setIsOpen] = useState(false);
...
const MobileDropdown = ({ title, items, filterType, customLinks = false, onLinkClick }) => {
  const [isOpen, setIsOpen] = useState(false);
...
const Navbar = () => {
  ...
  return (
    <>
      {/* Mobile menu overlay - FIXED: Now shows when menu is open */}
      {isMobileMenuOpen && (
        <div 
      ...
      >
        ...
        {/* FIXED: Mobile Menu - Now properly positioned and visible */}
        <div 
          className={`fixed top-16 left-0 right-0 bottom-0 bg-slate-900 z-50 transform transition-transform duration-300 ease-in-out overflow-y-auto lg:hidden ${

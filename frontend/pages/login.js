...
import { handleAuthError, _AuthError } from '../lib/errorHandler';
...
export default function SignInPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.error) {
        setError(handleAuthError(new Error(result.error)));
      } else {
  ...
  if (status === 'authenticated') {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-gradient-to-br from-indigo-900 via-blue-800 to-indigo-900">
        <div className="w-full max-w-md p-8 mx-4 backdrop-blur-lg bg-white/10 rounded-2xl shadow-2xl border border-white/20 text-white">
          <div className="mb-8 text-center">
            <h1 className="text-4xl font-bold text-white">
              Hello, {session.user.name || session.user.email}
            </h1>
            <p className="text-blue-100 mt-2">You are already signed in</p>
          </div>
          
          <div className="flex flex-col space-y-4">
            <button
              onClick={() => router.push('/')}
              className="w-full py-3 px-4 bg-white/20 text-white rounded-lg font-medium hover:bg-white/30 transition-all duration-300 backdrop-blur-sm"
            >
              Go to Home
            </button>
            ...
            <button
              ...
              className="w-full flex items-center justify-center py-3 px-4 bg-blue-500 text-white rounded-lg font-medium hover:bg-blue-600 transition-all duration-300 shadow-lg shadow-blue-500/30"
            >
              <span>Sign Out</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white/10 text-white border border-white/20 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition-all duration-300 placeholder-blue-200/70 backdrop-blur-sm"
            />
          </div>
